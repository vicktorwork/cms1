...
<script type="text/javascript">
    ...
    window.onload = function() {
        var settings = {
            ...
            upload_url: "{/literal}{$upload_url}{literal}",
            post_params: {"sess_id" :{/literal} "{$sess_id}", "album_id" : {$album.id}{literal}},
            file_size_limit: "20 MB",
            file_types: "*.jpg;*.png;*.gif;*.jpeg;*.JPG;*.PNG;*.GIF;*.JPEG",
            file_types_description: "{/literal}{$LANG.PHOTO}{literal}",
    {/literal}
    ...
    function queueComplete(numFilesUploaded) {
        if (numFilesUploaded>0){
            window.location.href = {/literal}'{$upload_complete_url}'{literal};
        }
    }
