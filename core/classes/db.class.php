...
// ============================================================================ //
	...
	protected function __construct(){
		...
		if(isset($_SESSION['lang']) && $_SESSION['lang'] != 'ru') { $this->db_prefix = $_SESSION['lang'] . '_'; }
		//if($_SESSION['not_found']) exit(var_dump($_SESSION['lang_not_found']));
	}
...
// ============================================================================ //
	...
	protected function replacePrefix_new($sql, $prefix='cms_', $new_prefix='cms_'){
        if($prefix == $this->db_prefix){
            return trim($sql);
        }
		//return trim(str_replace($prefix, $this->db_prefix, $sql));
		return trim(str_replace($prefix, $new_prefix, $sql));
	}
...
//echo $sql;
	...
	public function query_new($sql, $ignore_errors=false, $replace_prefix = true, $prefix='cms_', $new_prefix='cms_'){
	
        if (empty($sql)) { return false; }
		
        $sql = $replace_prefix ? $this->replacePrefix_new($sql, $prefix, $new_prefix) : $sql;
		//echo $sql;
		$result = mysqli_query($this->db_link, $sql);
...
// ============================================================================ //
	...
	public function get_fields_lang($table, $where, $fields='*', $order='id ASC'){
		...
		if ($this->num_rows($result)){
			...
			foreach($prefix_mas as $prefix){
				$result = $this->query_new($sql, false, true, 'cms_', $prefix);
				//echo "<pre>"; var_dump($sql);
				//echo "<pre>"; var_dump($result);
				//echo "<pre>"; var_dump($prefix);
				if ($this->num_rows($result)) {
					$cat_info = $this->fetch_assoc($result);
					$cat_id = $cat_info['id'];
...
// ============================================================================ //
	...
	public function getNsCategory($table, $cat_id_or_link=0, $differ='') {
		...
		$cat = $this->get_fields_lang($table, $where, '*');

		return $cat ? $this->cache[$table][$cat_id_or_link][$differ] = $cat : false;

	}
	
	public function getNsCategory_back($table, $cat_id_or_link=0, $differ='') {
