...
define('CORE_VERSION', 		'1.10.5');
define('CORE_BUILD', 		'2');
define('CORE_VERSION_DATE', '2014-11-18');
define('CORE_BUILD_DATE', 	'2014-11-21');
...
class cmsCore {
        ...
        foreach($plugins as $plugin_name){
            ...
            if ($plugin!==false){
				...
				if(isset($plugin->info['type'])){
					if (in_array($plugin->info['plugin_type'], self::getInstance()->single_run_plugins)) {
						return $item;
					}
				}
            }
    ...
    public static function getListItems($table, $selected=0, $order_by='id', $order_to='ASC', $where='', $id_field='id', $title_field='title'){
	 ...
	 public static function getListfiles($table, $selected=0, $order_by='id', $order_to='ASC', $where='', $id_field='filename', $title_field='filename'){
        $inDB = cmsDatabase::getInstance();
        $html = '';
        $sql  = "SELECT {$id_field}, {$title_field} FROM {$table} \n";
        if ($where){
        ...
        while($item = $inDB->fetch_assoc($result)){
            ...
            //$html .= '<option value="'.PATH.'/upload/userfiles/'.$inUser->id.htmlspecialchars($item[$id_field]).'" '.$s.'>'.$item[$title_field].'</option>';
           // $html .= '<option value="/upload/userfiles/1/'.htmlspecialchars($item[$id_field]).'" '.$s.'>'.$item[$title_field].'</option>';
           // $html .= '<option value="/upload/userfiles/1/'.$item[$title_field].'" '.$s.'>'.$item[$title_field].'</option>';
            $html .= '<option value="/upload/userfiles/1/'.$item[$id_field].'" '.$s.'>'.$item[$title_field].'</option>';
        }
        return $html;
    }
