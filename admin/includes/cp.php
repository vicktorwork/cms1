...
function cpMenu(){
	...
	<div id="hmenu">
		<ul id="nav">
			...
			<?php } ?>
			<li>
				<a href="../wiki.html" target="_blank" class="help"><?php echo $_LANG['AD_DOCS']; ?></a>
			</li>
		</ul>
	</div>
		...
		echo '<table id="listTable" border="0" class="tablesorter" width="100%" cellpadding="0" cellspacing="0">';
			...
			while ($item = $inDB->fetch_assoc($result)){
				...
				echo '<tr id="lt_row2">'."\n";
					...
					foreach($_fields as $key => $value){
					/*echo "<pre>"; var_dump($_fields);
					var_dump($key);
					var_dump($value);*/
						if (isset($_fields[$key]['link'])){
							$link = str_replace('%id%', $item['id'], $_fields[$key]['link']);
							
							/* if($_fields[$key]['field'] == 'image'){
								$link = str_replace('%photo%', $item['file'], $_fields[$key]['link']);
							} */
							...
							if (isset($_fields[$key]['maxlen'])){
							  ...
							  //nested sets otstup
							if (isset($item['NSLevel']) && $_fields[$key]['field']=='filename'){
						...
						elseif (isset($_fields[$key]['image'])){
							if($_fields[$key]['field'] == 'image'){
								$photo_url = str_replace('%photo%', $item['file'], $_fields[$key]['image']);
							}
							echo '<td class="'.$row_class.'" valign="middle"><a class="photobox" target="_blank" href="'.$photo_url.'"><img width="32" height="20" src="'.$photo_url.'"/></a></td>'. "\n";
						} else {
							...
							/* }
							elseif ($_fields[$key]['field'] == 'image'){
								echo "asdasdas"; */
							} else {
...
echo '<table width="100%" border="0" cellspacing="0" cellpadding="8" class="proptable"><tr><td>';
	...
	echo '<tr id="filelink">';
        ...
        echo '<td>
					<select name="fl" style="width:99%">'.$inCore->getListfiles('cms_user_files',0,'id','ASC',1,'filename','title' ).'</select>
					<!--select name="fl" style="width:99%">'.$inCore->getListfiles('cms_user_files',0,'id','ASC',1 ).'</select-->
