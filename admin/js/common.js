...
function insertTag(kind){
    ...
    if (kind=='filelink'){
        text = '{СКАЧАТЬ=' + document.addform.fl.value + '---'+ document.addform.fl.options[document.addform.fl.selectedIndex].text +'}';
    }
    if (kind=='banpos'){
...
/* function submit_title(){
	...
	$.ajax({
		...
		success: function(msg){
			if(msg){
				$('img#pub'+id).attr('src', 'images/actions/'+action+'.gif');
				$('a#publink'+id).attr('href', 'javascript:pub1('+id+', "'+qs2+'", "'+qs+'", "'+action2+'", "'+action+'");');
			} else {
				$('img#pub'+id).attr('src', old_img);
			}
		}
