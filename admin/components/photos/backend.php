...
//=================================================================================================//
	...
	if($opt == 'delete_item'){
		...
		$photo_id = cmsCore::request('photo_id', 'int', 0);
		$inDB->query("DELETE FROM cms_photo_files WHERE id = '$photo_id'") ;
...
//=================================================================================================//
	...
	if ($opt == 'add_album' || $opt == 'edit_album'){
		...
		function css(){
			$('#frame_1').contents().find('#header').css('display','none');
			$('#frame_1').contents().find('#topmenu').css('display','none');
			$('#frame_1').contents().find('#pathway').css('display','none');
			$('#frame_1').contents().find('#footer').css('display','none');
			$('#frame_1').contents().find('body').css('background','none');
			//$('#frame_1').contents().find('body').css('overflow','hidden');
			$('#frame_1').contents().find('.usr_photos_notice').css({'width':'75%', 'word-wrap':'break-word'});
		}
		function show_frame(){
		...
		cssLink.href = "style_iframe.css"; 
		cssLink .rel = "stylesheet"; 
		cssLink .type = "text/css"; 
		frames['frame_1'].document.body.appendChild(cssLink); */
        </script>
		<div id="bg_"></div>
		<iframe name="frame_1" id="frame_1" frameborder="1"> </iframe>
		
		<div id="photos_tabs" class="uitabs">
		
		<ul id="tabs">
			<li><a href="#photos_list"><span><?php echo $_LANG['AD_PHOTOS_LIST']; ?></span></a></li>
			<li><a href="#settings"><span><?php echo $_LANG['AD_PHOTOS_SETTINGS']; ?></span></a></li>
		</ul>
		
		<div id="photos_list">
			<?php
				$GLOBALS['cp_page_head'][] = '<script type="text/javascript" src="/admin/components/photos/js/photos.js"></script>';
				cpAddPathway($_LANG['AD_ITEMS']);
				$album_id = cmsCore::request('item_id', 'int', 0);
				
				echo '<div id="add_photos"><a href="/photos/' . $album_id . '/addphoto.html" target="frame_1" onclick="show_frame();">Добавить фото в альбом</a></div>';
				...
				//$actions[] = array('title'=>$_LANG['DELETE'], 'icon'=>'delete.gif', 'confirm'=>$_LANG['AD_IF_ITEM_DELETE'], 'confirm'=>$_LANG['AD_IF_ITEM_DELETE'], 'link'=>'?view=components&do=config&id='.$id.'&opt=delete_item&photo_id=%id%');
				$actions[] = array('title'=>$_LANG['DELETE'], 'icon'=>'delete.gif', 'confirm'=>$_LANG['AD_IF_ITEM_DELETE'], 'confirm'=>$_LANG['AD_IF_ITEM_DELETE'], 'link'=>'?view=components&do=config&id='.$id.'&opt=delete_item&photo_id=%id%');
				
				if (cmsCore::inRequest('on_moderate')){ $where = 'on_moderate=1'; } else { $where = ''; }
				$where = "album_id={$album_id}";
				
				cpListTable('cms_photo_files', $fields, $actions, $where);
			?>
		</div>
		<div id="settings">
